...
Then the probability/risk of an uninfected individual being tested is governed by the baseline hazard $b$: $$ T_B=r(b)=1-e^{-b} $$ And the probability/risk of an infected individual being tested is governed by the baseline hazard $b$ and the offset $\phi$: $$ T_V=r(b+\phi)=1-e^{-(b+\phi)} $$ For simplicity we denote $B=e^{-b}$ and $\Phi=e^{-\phi}$, then we have $$ T_B=1-B $$ $$ T_V=1-B\Phi$$ Then at any time, the testing proportion $T$, also as the probability of a random individual being tested, can be expressed as: $$ T=T_V(1-V)+T_B V=(1-V)(1-B)+V(1-B\Phi)$$ This gives us $B$ (thus $b=-log(B)$ as the baseline hazard) $$B(V,T,\Phi)=\frac{1-T}{1-V(1-\Phi)}$$ Consider we can observe testing positivity $P$ and testing proportion $T$ from data. $B$ (thus $b$) is a function depend on 3 states: prevalence $V(t)$, testing proportion $T(t)$ and $\phi$, s.t. $B(t)=B(T,V,\Phi)$.

(To be discussed) $B$ can be seen as the behavioral(???) factor reflect public and administrative reaction to the endemic. They partially observe $V$, and the testing probability is also limited by testing availability reflected by $T$. While $\Phi$ (thus $\phi$) is more about medical/biological factor of the infection, like the severity/clarity of the symptoms.

$T$ increase leads to $B$ decrease, $T_B=1-B$ increase: more testing available, more people get tested in general. $V$ increase leads to $B$ increase, $T_B=1-B$ decrease: more prevalence make uninfected people get less probability of being tested. $\phi$ increase leads to $\Phi=e^{-\phi}$ decrease, then $B$ increase, $T_B=10B$ decrease: the symptom of the disease is more clear/severe, thus infected people is more likely of being tested, while uninfected people is less likely.

Current simplify idea is assume $\phi$ is a constant for a certain period of time and in future it could be a function of $t$.

The testing positivity $P$ can be expressed as: $$ P = \frac{V T_V}{T}=\frac{V(1-B\Phi)}{T}$$ \## Odds Ratio
