...
-   Prevalence at time $t$ is $$Y(t)=\text{min}(Y_0 e^{rt},1)$$ to make sure it does not go exceed $1$ for large $r$.
-   The number of infected population $N_{+}$ is binomial distributed with probability $Y$: $$N_{+}\sim \text{Binom}(N,Y)$$
...
-   The number of negative tests $N T_{-}$ is just the number of negative people tested. $$NT_{-} \sim \text{Binom}(N(1-Y), T_B)$$
-   $T=T_{+} + T_{-}$
...
This clarifies also that the most likely way for the model to fall off the cliff would be if the number of positive tests $NT^{*}_{+}$ (from the “data”) exceeds number of positive people $N*Y$ (from the current simulation). This could happen if $r<<r^{*}$ and/or $Y_0 << Y_0^{*}$ in exponential growth. - (??)Generate an error message.

It's not obvious what is the best way to simulate number infected (Number of positive $N_{+}$): we want it to be a whole number. Seems almost easier to simulate with dynamical noise (formerly known as process error), but I'm sticking with binomial observation noise for now.
