...
library(DPQ)
library(deSolve)
source("prop_pos_test_new.R")

### Numeric Check for explicity soln of p(t)
a<-1
...
abs_log_sub <- function(x,y,zeroHL=-Inf){
  ...
  ## Exclude failing case: possible reason, Qbeta gives 1
  ##### FIXIT??: 
  if (is.infinite(sgn)){
    out <- NaN
    } else if(is.nan(sgn)){
...
#prop_pos_test_new(0.3,0.001,0.99,method = "log",debug=T)
