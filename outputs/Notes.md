...
-   $\Phi$ is the (constant) odds ratio between test probabilities of positive and negative people. $$\Phi=\frac{\frac{T_Y}{1-T_Y}}{\frac{T_B}{1-T_B}}=\frac{\frac{T_Y}{1-T_Y}}{B} \Leftrightarrow \Phi B= \frac{T_Y}{1-T_Y} \Leftrightarrow T_Y=\frac{B\Phi}{1+B\Phi}$$

    -   **Constraint** $\Phi>0$
    -   The probalibity $T_B$ is monotonically increase with corresponding odds.

-   (??) Similar like hazard ratio, did the probability/odds of baseline/negative people a constant? It seems should be corresponding to the behavior factors.
